...
const Login = () => {
  ...
  function HandleSubmit(event) {
    ...
    if(formData.email.length == 0 && formData.password.length == 0){
      setWarning(<h2 class="text-sm font-bold text-white bg-red-600 mb-4 p-2 rounded">Please Enter Email And Password</h2>);
      return;
    }
    else if(formData.email.length == 0){
      setWarning(<h2 class="text-sm font-bold text-white bg-red-600 mb-4 p-2 rounded">Please Enter Email</h2>);
      return;
    }
    else if(formData.password.length == 0){
      setWarning(<h2 class="text-sm font-bold text-white bg-red-600 mb-4 p-2 rounded">Please Enter Password</h2>);
      return;
    }
      ...
      <div class="flex flex-col items-center justify-center h-screen">
        <div class="w-full max-w-md bg-white rounded-lg ring-1 ring-gray-200 shadow-xl p-6">
          ...
          <form class="flex flex-col" onSubmit={HandleSubmit}>
            <input type="email" name="email" onChange={HandleChange} class="bg-gray-100 text-gray-900 border-0 rounded-md p-2 mb-4 focus:bg-gray-200 focus:outline-none focus:ring-1 focus:ring-blue-500 transition ease-in-out duration-150" placeholder="Email address" />
            <input type="password" name="password" onChange={HandleChange} class="bg-gray-100 text-gray-900 border-0 rounded-md p-2 mb-4 focus:bg-gray-200  focus:outline-none focus:ring-1 focus:ring-blue-500 transition ease-in-out duration-150" placeholder="Password" />
